...
void test(unsigned char plain[10000]){
	...
	for(i=0;i<part;i++){
		for(j=0;j<16;j++){
			...
			block[j]=(plain[j*4]<<24)+(plain[j*4+1]<<16)+(plain[j*4+2]<<8)+plain[j*4+3];
		}
		encrypt(block);
	}
	//for(i=0;i<8;i++){
...
int main(void){
	...
	for(i=1;i<5;i++){
		test(chain[i-1]);
		for(j=0;j<8;j++){
			chain[i][j*4]=(h[j]>>24)&0xff;
			chain[i][j*4+1]=(h[j]>>16)&0xff;
			chain[i][j*4+2]=(h[j]>>8)&0xff;
			chain[i][j*4+3]=(h[j])&0xff;
		}
		chain[i][32]='d';
		chain[i][33]='a';
		chain[i][34]='t';
		chain[i][35]='a';
		chain[i][36]='0'+i;
		//chain[1][36]='2';little change
		cout<<"chain["<<i<<"]:";
		for(j=0;j<=36;j++){
			printf("%02x",chain[i][j]);
		}
		cout<<endl;
	}
