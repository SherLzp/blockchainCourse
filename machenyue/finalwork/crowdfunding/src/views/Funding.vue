  ...
  <div>
    ...
    >
      ...
      <a-descriptions bordered v-if="key==='info'">
        ...
        <a-descriptions-item label="项目发起人" :span="2">
        ...
        <a-descriptions-item label="目标金额（ETH）" :span="2">
          {{state.data.goal}}
        </a-descriptions-item>
        <a-descriptions-item label="当前金额（ETH）" :span="2">
          {{state.data.amount}}
        </a-descriptions-item>
        <a-descriptions-item label="当前状态">
          <a-text v-if="state.data.success === true">
            已成功
          </a-text>
          <a-text v-else-if="new Date(state.data.endTime * 1000) > new Date()" >
            进行中
          </a-text>
          <a-text v-else>
            已失败
    ...
    >
      <h3>
        <span style="float:center">
           <br/>
          你已投资 {{state.myAmount}} Eth
        </span>
      </h3>
      <h4>
        <span style="float:center">
          <br/>
          <a-button type="primary" v-if="new Date(state.data.endTime * 1000) > new Date() && state.data.success == false" @click="openModal">我要参与众筹</a-button>
        </span>
      </h4>
    </a-card>
