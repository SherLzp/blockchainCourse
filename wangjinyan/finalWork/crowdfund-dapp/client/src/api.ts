...
const add = '0xCECB04823d580d85A0719a49F448ea6729d2Cb41';


const contract = new web3.eth.Contract(CrowdFunding.abi, add);

export declare interface Fundings {
    account: string,
    title: string,
    info: string,
    goal: number,
...
async function getAllFundings() : Promise<Funding[]> {
    const length = await contract.methods.fundingsCount().call();
    const result = []
...
async function newFunding(account:string, title:string, info:string, amount:number,seconds1:number,seconds2:number) {
    return await contract.methods.newFunding(account, title, info, Web3.utils.toWei(amount.toString(10), 'ether'), seconds1,seconds2).send({
        from: account,
        gas: 1000000
    });
}
...
async function newUse(id:number, goal:number, info:string) {
    ...
    return await contract.methods.Apply(id, eth, info).send({
        from: account,
        gas: 1000000
    })
}
...
async function returnMoney(id: number) {
    const account = await getAccount();
    return await contract.methods.giveBack(id).send({
        from: account,
        gas: 1000000
    })
}
