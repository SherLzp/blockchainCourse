...
var fs = require('fs');
var web3 = require('./Lianjie');
var factoryABI = fs.readFileSync(path.join(__dirname, '../Solidity/build/contracts/ZhongChou.json')).toString();
var FactoryContract = web3.eth.contract(JSON.parse(factoryABI).abi);
var fundingABI = fs.readFileSync(path.join(__dirname,'../Solidity/build/contracts/ChouQian.json')).toString();
var fundingContract = web3.eth.contract(JSON.parse(fundingABI).abi);
const contractAddr = 'ac3dC4A7315b4e64A88f7a090B5219a62e59b8Ac';//存放合约地址
...
module.exports = {
    ...
    //获取当前账号
    async get_curaccounts(user) {
        return user;
    },
    async getBalance (users) {
        return web3.eth.getBalance(users);
    },
    //创建众筹
