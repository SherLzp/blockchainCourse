...
contract CrowdFunding {
    ...
    constructor(string memory _title, address payable _initiator, uint _endTime, uint _goal, string memory _info) public {
        title = _title;
...
contract AllFundings {
    ...
    // 创建众筹项目
    function newFunding(string calldata title, uint endTime, uint goal, string calldata info) public{
        CrowdFunding funding = new CrowdFunding(title, msg.sender, endTime, goal, info);
    ...
    // 获取某众筹截止时间
    function getEndTime(uint i) public view returns(uint){
        return CrowdFunding(fundings[i]).endTime();
    }
